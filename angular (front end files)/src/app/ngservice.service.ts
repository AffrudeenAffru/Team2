...
//
fetchsubreq():Observable<any>{
  return this._http.get<any>("http://localhost:4321/subscribersptop");
}
//marketting msg
sendmarkettingmsg(Num)
  {console.log("from servoice"+Num );
      return this._http.get<any>("http://localhost:1234/txt/"+Num).subscribe(
        data => console.log("msg sent", data),
        error => console.error("Error in req", error))
  }

  //marketing call
  sendmarkettingcall(Num){
    console.log("from servoice"+Num);
    return this._http.get<any>("http://localhost:1234/voicecall/"+Num).subscribe(
      data => console.log("caling", Num),
